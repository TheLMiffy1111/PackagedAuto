...
public class MiscHelper implements IMiscHelper {
	...
	public List<ItemStack> condenseStacks(List<ItemStack> stacks, boolean ignoreStackSize) {
		Object2LongLinkedOpenHashMap<Pair<Item, CompoundNBT>> map = new Object2LongLinkedOpenHashMap<>();
		...
		for(Object2LongMap.Entry<Pair<Item, CompoundNBT>> entry : map.object2LongEntrySet()) {
	...
	public List<IPackageRecipeInfo> readRecipeList(ListNBT tagList) {
		...
		for(int i = 0; i < tagList.size(); ++i) {
			IPackageRecipeInfo recipe = readRecipe(tagList.getCompound(i));
			if(recipe != null) {
				recipes.add(recipe);
			}
		}
	...
	public boolean removeExactSet(List<ItemStack> offered, List<ItemStack> required, boolean simulate) {
		...
		f:for(ItemStack req : condensedRequired) {
			for(ItemStack offer : condensedOffered) {
				if(req.getCount() <= offer.getCount() && req.sameItem(offer) &&
		...
		for(ItemStack req : condensedRequired) {
			...
			for(ItemStack offer : offered) {
				if(!offer.isEmpty()) {
					if(req.sameItem(offer) && (!req.hasTag() || ItemStack.tagMatches(req, offer))) {
