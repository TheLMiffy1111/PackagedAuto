		...
		for(int i = 0; i < 6; ++i) {
			for(int j = 0; j < 3; ++j) {
				if(patternInventory.recipeType.canSetOutput()) {
					addSlotToContainer(new SlotFalseCopy(patternInventory, 81+i*3+j, 198+j*18, 57+i*18));
				}
				else {
					addSlotToContainer(new SlotPreview(patternInventory, 81+i*3+j, 198+j*18, 57+i*18));
				}
			}
		}
		for(int i = 0; i < 3; ++i) {
			for(int j = 0; j < 3; ++j) {
				addSlotToContainer(new SlotPreview(patternInventory, 99+i*3+j, 198+j*18, 165+i*18));
			}
		}
